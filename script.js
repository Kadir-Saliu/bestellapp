let myDishes = [
  ...
  },
  {
    ...
    price: 9.50,
    description: "mit Tomatensoße, Gouda, Hackfleich und Zwiebel ",
  },
];
...
function getInformationTemplate(indexNote) {
  return `<div class="menu" >
        <div>
            ...
            <p> <strong> Preis: </strong> ${myDishes[indexNote].price.toFixed(
              2
            )} €</p>
            <p> <strong> Beschreibung: </strong> ${
              myDishes[indexNote].description
            }</p>
        </div>
            <div>
...
function getBasketTemplate(indexBasket) {
  ...
  <div  class="basket-menu" >
          ...
          <div>
              <p id="ammountPriceUp${indexBasket}"> ${
    basketCarts[indexBasket].amount
  } x</p>
              <h3> ${basketCarts[indexBasket].name}</h3>
              <p id='changePrice${indexBasket}'> ${
    basketCarts[indexBasket].price.toFixed(2) * basketCarts[indexBasket].amount
  } €</p>
...
function minusAmmountPrice(indexBasket) {
  ...
  if (basketCarts[indexBasket].amount < 1) {
    basketCarts.splice(indexBasket, 1);
...
function cost() {
  ...
  currentCost.innerHTML = "Zwischensumme: " + sum.toFixed(2) + " €";
  console.log("sum: ", sum);
  let deliveryCost = sum > 0 ? 5 : 0
  deliveryCosts.innerHTML = ` Lieferkosten: ${deliveryCost.toFixed(2)} €`;

  total.innerHTML = `Gesamtpreis: ${sum + deliveryCost} €`;
